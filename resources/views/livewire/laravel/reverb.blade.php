...
<div class="h-[90vh] flex flex-col space-y-6">
    ...
    <div class="flex-1 flex flex-col border border-gray-300 dark:border-gray-700 rounded-xl shadow-md overflow-hidden bg-white dark:bg-zinc-800" style="min-height:600px;">
        <div class="flex-1 overflow-y-auto p-4 space-y-2">
            @foreach($messages as $message)
                ...
                @else
                    <div class="flex">
                        <div class="bg-gray-100 dark:bg-zinc-700 border border-gray-400 dark:border-gray-600 text-gray-800 dark:text-gray-100 p-2 rounded-lg rounded-bl-none max-w-xs text-sm">
                            {{$message['message']}}
                        </div>
                    </div>
                    <p class="text-[8px] -mt-1 text-gray-600 dark:text-gray-400">{{$message['user'] ?? 'Unknown'}}</p>
                @endif
            @endforeach
            <flux:spacer />
        </div>

 ...
 * import { camera, dialog, edge, scanner } from '@nativephp/native';
 *
 * // Show alert dialog
 ...
 * import { getPhoto, alert, edgeBuilder, scanner } from '@nativephp/native';
 *
 ...
 * });
 *
 * @example Edge UI Builder - Bottom Navigation
 * import { edgeBuilder } from '@nativephp/native';
 *
 * await edgeBuilder()
 *   .bottomNav({ dark: true })
 *     .bottomNavItem({
 ...
 *   .build();
 *
 * @example Edge UI Builder - Top Bar with Actions
 * import { edgeBuilder } from '@nativephp/native';
 *
 * await edgeBuilder()
 *   .topBar({
 ...
 *   .build();
 *
 * @example Edge UI Builder - Side Navigation with Groups
 * import { edgeBuilder } from '@nativephp/native';
 *
 * await edgeBuilder()
 *   .sideNav({ dark: false, gesturesEnabled: true })
 *     .sideNavHeader({
 *       title: 'John Doe',
 ...
 *     })
 *     .horizontalDivider()
 *     .sideNavGroup({ heading: 'Account', expanded: true })
 *       .sideNavItem({
 *         id: 'profile',
 *         label: 'Profile',
 *         url: '/profile',
 ...
 *   .build();
 *
 * @example Edge UI Builder - Multiple Components
 * import { edgeBuilder } from '@nativephp/native';
 *
 * await edgeBuilder()
 *   .topBar({ title: 'Home' })
 *   .end()
 *   .bottomNav()
 *     .bottomNavItem({ id: 'home', icon: 'home', url: '/', label: 'Home' })
 *     .bottomNavItem({ id: 'explore', icon: 'explore', url: '/explore', label: 'Explore' })
 *   .end()
 *   .fab({ icon: 'add', url: '/create', position: 'end' })
 *   .build();
 *
 * @example Direct Edge Set (without builder)
 * import { setEdge } from '@nativephp/native';
 *
 * await setEdge([
 *   {
 *     type: 'bottom_nav',
 ...
 *           data: { id: 'home', icon: 'home', url: '/', label: 'Home' }
 *         }
 *       ]
 *     }
 *   }
...
/**
 * EdgeBuilder - Fluent API for building native UI component trees
 */
class EdgeBuilder {
    constructor() {
        this.components = [];
     ...
     * @private
     */
    _addComponent(type, data, hasChildren = false) {
        const component = { type, data };

        if (hasChildren) {
            // Store current state and start collecting children
            component.data.children = [];
            this.contextStack.push(component);
        } else {
            // Add to current context or root
            if (this.contextStack.length > 0) {
                const parent = this.contextStack[this.contextStack.length - 1];
                parent.data.children.push(component);
            } else {
    ...
    /**
     * End the current context (close a parent component)
     * @private
     */
    _endContext() {
        if (this.contextStack.length > 0) {
            const component = this.contextStack.pop();
     ...
     * @param {object} options - { dark: boolean, labelVisibility: string }
     * @returns {EdgeBuilder}
     */
    bottomNav(options = {}) {
        return this._addComponent('bottom_nav', {
            dark: options.dark ?? null,
            label_visibility: options.labelVisibility ?? 'labeled',
            id: 'bottom_nav',
            children: []
        }, true);
    }
     ...
     * @param {object} options - { id, icon, url, label, active, badge, badgeColor, news }
     * @returns {EdgeBuilder}
     */
    bottomNavItem(options) {
        return this._addComponent('bottom_nav_item', {
            id: options.id,
            icon: options.icon,
            url: options.url,
            label: options.label,
            ...
            badge_color: options.badgeColor ?? null,
     ...
     * @param {object} options - { title, subtitle, showNavigationIcon, backgroundColor, textColor, elevation }
     * @returns {EdgeBuilder}
     */
    topBar(options) {
        const data = {
        ...
        if (options.subtitle !== undefined) data.subtitle = options.subtitle;
        if (options.showNavigationIcon !== undefined) data.show_navigation_icon = options.showNavigationIcon;
        if (options.backgroundColor !== undefined) data.background_color = options.backgroundColor;
        if (options.textColor !== undefined) data.text_color = options.textColor;
        if (options.elevation !== undefined) data.elevation = options.elevation;

        return this._addComponent('top_bar', data, true);
    }

    /**
     * Top Bar Action Button
     * @param {object} options - { id, icon, label, url, event }
     * @returns {EdgeBuilder}
     */
    topBarAction(options) {
        return this._addComponent('top_bar_action', {
            id: options.id,
            icon: options.icon,
     ...
     * @param {object} options - { icon, label, url, event, size, position, bottomOffset, elevation, cornerRadius, containerColor, contentColor }
     * @returns {EdgeBuilder}
     */
    fab(options) {
        const data = { icon: options.icon };

        if (options.label !== undefined) data.label = options.label;
        if (options.url !== undefined) data.url = options.url;
        if (options.event !== undefined) data.event = options.event;
        if (options.size !== undefined) data.size = options.size;
        if (options.position !== undefined) data.position = options.position;
        if (options.bottomOffset !== undefined) data.bottom_offset = options.bottomOffset;
        if (options.elevation !== undefined) data.elevation = options.elevation;
        if (options.cornerRadius !== undefined) data.corner_radius = options.cornerRadius;
        if (options.containerColor !== undefined) data.container_color = options.containerColor;
        if (options.contentColor !== undefined) data.content_color = options.contentColor;

        return this._addComponent('fab', data);
    }

    /**
     * Side Navigation Drawer
     * @param {object} options - { dark, labelVisibility, gesturesEnabled }
     * @returns {EdgeBuilder}
     */
    sideNav(options = {}) {
        return this._addComponent('side_nav', {
            dark: options.dark ?? null,
            label_visibility: options.labelVisibility ?? 'labeled',
            gestures_enabled: options.gesturesEnabled ?? false,
            children: []
        }, true);
    }
     ...
     * @param {object} options - { title, subtitle, icon, backgroundColor, imageUrl, event, showCloseButton, pinned }
     * @returns {EdgeBuilder}
     */
    sideNavHeader(options = {}) {
        const data = {};
        ...
        if (options.backgroundColor !== undefined) data.background_color = options.backgroundColor;
        if (options.imageUrl !== undefined) data.image_url = options.imageUrl;
        if (options.event !== undefined) data.event = options.event;
        if (options.showCloseButton !== undefined) data.show_close_button = options.showCloseButton;
        if (options.pinned !== undefined) data.pinned = options.pinned;

        return this._addComponent('side_nav_header', data);
    }

    /**
     * Side Navigation Item
     * @param {object} options - { id, label, url, icon, active, badge, badgeColor, openInBrowser }
     * @returns {EdgeBuilder}
     */
    sideNavItem(options) {
        const data = {
        ...
        if (options.badge !== undefined) data.badge = options.badge;
        if (options.badgeColor !== undefined) data.badge_color = options.badgeColor;
        if (options.openInBrowser !== undefined) data.open_in_browser = options.openInBrowser;

        return this._addComponent('side_nav_item', data);
    }

    /**
     * Side Navigation Group (collapsible section)
     * @param {object} options - { heading, expanded, icon }
     * @returns {EdgeBuilder}
     */
    sideNavGroup(options) {
        const data = {
            heading: options.heading,
            children: []
        };

        if (options.expanded !== undefined) data.expanded = options.expanded;
        if (options.icon !== undefined) data.icon = options.icon;

        return this._addComponent('side_nav_group', data, true);
    }
     ...
     */
    horizontalDivider() {
        return this._addComponent('horizontal_divider', {});
    }

    /**
     * End the current parent component context
     * @returns {EdgeBuilder}
     */
    end() {
    ...
    /**
     * Build and send the component tree to native
     * @returns {Promise<any>}
     */
    async build() {
        ...
        return setEdge(this.components);
    }

    /**
     * Get the built component tree without sending
     * @returns {Array}
     */
    toJSON() {
        // Close any remaining open contexts
...
/**
 * Create a new EdgeBuilder for fluent component creation
 * @returns {EdgeBuilder}
 */
 ...
 * on(NativeEvents.Alert.ButtonPressed, handler);
 */
